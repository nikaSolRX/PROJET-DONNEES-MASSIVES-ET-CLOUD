...
PARAMS = [10, 100, 1000]   # posts par user (en moyenne)
RUNS = 3
TOTAL_REQUESTS = 1000
CONCURRENCY = 50
...
def seed_posts():
    print("==> SEED posts10 (10 posts/user, follows=20)")
    subprocess.run([
        "python", "seed.py",
        "--users", "1000",
        "--posts", "10000",
    ...
    print("==> SEED posts100 (100 posts/user, follows=20)")
    subprocess.run([
        "python", "seed.py",
        "--users", "1000",
        "--posts", "100000",
    ...
    print("==> SEED posts1000 (1000 posts/user, follows=20)")
    subprocess.run([
        "python", "seed.py",
        "--users", "1000",
        "--posts", "1000000",
...
def run_for_param(param: int, run_idx: int):
    ...
    users = users_for_param(param)  # 50 users
    c = len(users)
...
def make_bar_chart(rows):
    ...
    plt.xlabel("Nombre de posts par utilisateur (PARAM)")
    plt.ylabel("Temps moyen par requête (ms)")
    plt.title("Performance vs nombre de posts (C=50)")
    plt.tight_layout()
